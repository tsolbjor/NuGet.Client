...
namespace NuGet.Commands
{
    internal static class LockFileUtils
    {
            ...
            RuntimeGraph runtimeGraph)
        {
            ...
            var contentFileGroupsForFramework = GetContentGroupsForFramework(
                lockFileLib,
                framework,
                contentFileGroups,
            ...
            // Runtime targets
            // These are applied to non-RID target graphs
            if (string.IsNullOrEmpty(runtimeIdentifier))
            {
                // Runtime targets contain all the runtime specific lock file items
                // that would be contained in runtime specific target graphs.
                // These items are contained in a flat list and have additional properties 
                // the for the RID and lock file section the assembly would belong to.
                ...
                runtimeTargetItems.AddRange(GetRuntimeDependencies(runtimeGraph, package.Id));
            ...
            string groupLabel)
        {
            ...
            var items = GetRuntimeTargetItems(groupsForFramework, groupLabel);
        ...
        private static List<LockFileItem> GetRuntimeTargetItems(List<ContentItemGroup> groups, string groupLabel)
        {
            ...
            foreach (var group in groups)
            {
                ...
                foreach (var item in group.Items)
                {
                    ...
                    lockFileItem.Properties.Add("group", groupLabel);
        ...
        private static List<LockFileItem> GetRuntimeDependencies(RuntimeGraph runtimeGraph, string packageId)
        {
            var runtimeTargetItems = new List<LockFileItem>();

            if (runtimeGraph != null)
            {
                foreach (var dependencyGroup in runtimeGraph.FindAllRuntimeDependencies(packageId))
                {
                    var rid = dependencyGroup.Key;

                    foreach (var runtimeDependency in dependencyGroup.Value)
                    {
                        var lockFileItem = new LockFileItem(runtimeDependency.Id);
                        lockFileItem.Properties.Add(ManagedCodeConventions.PropertyNames.RuntimeIdentifier, rid);
                        lockFileItem.Properties.Add("group", "dependencies");
                        lockFileItem.Properties.Add("version", runtimeDependency.VersionRange.ToLegacyShortString());

                        runtimeTargetItems.Add(lockFileItem);
                    }
                }
            }

            return runtimeTargetItems;
        }
    }
}
