            ...
            IEnumerable<string> targetIds)
        {
            ...
            if (!isReinstall)
            {
                if (targetIds.Any())
                {
                    ...
                    var newTargetIds = new List<string>(newPackagesToUninstall.Select(p => p.Id));
            ...
            CancellationToken token)
        {
            ...
            if (resolutionContext.DependencyBehavior != DependencyBehavior.Ignore)
            {
                try
                {
                    ...
                    // this will be the new set of target ids for which we should get all the dependencies
                    var newTargets = new List<PackageIdentity>(newPackagesToUninstall);
                    newTargets.Add(packageIdentity);
                    var newTargetIds = newPackagesToUninstall.Select(p => p.Id);
