...
namespace NuGet.Resolver
{
    ...
    public class PackageResolver
    {
        ...
        public IEnumerable<PackageIdentity> Resolve(PackageResolverContext context, CancellationToken token)
        {
            ...
            var invalidsPackages = new List<string>();
            ...
            foreach (var package in installedPackages)
            {
                ...
                if (existingPackage != null)
                {
                    ...
                    {
                        invalidsPackages.AddRange(brokenDependencies.Select(dependency => FormatDependencyConstraint(existingPackage, dependency)));
                    }
                }
            }
            // log warning message for all the invalid package dependencies
            if (invalidsPackages.Count > 0)
            {
                context.Log.LogWarning(
                    string.Format(
                        CultureInfo.CurrentCulture, Strings.InvalidPackageConfig, string.Join(", ", invalidsPackages)));
            }
