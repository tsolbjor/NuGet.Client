    ...
    public class VsPackageInstaller : IVsPackageInstaller2
    {
            ...
            CancellationToken token)
        {
            ...
            try
            {
                ...
                {
                    var alreadyInstalled = new List<PackageIdentity>((await nuGetProject.GetInstalledPackagesAsync(token))
                        .Select(reference => reference.PackageIdentity));

                    if (package.Version == null)
                    {
                        if (alreadyInstalled.Any(reference => StringComparer.OrdinalIgnoreCase.Equals(package.Id, reference.Id)))
                        {
                            continue;
                        }
                    }
