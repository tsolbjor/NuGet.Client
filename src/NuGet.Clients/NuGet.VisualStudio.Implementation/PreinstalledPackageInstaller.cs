    ...
    internal class PreinstalledPackageInstaller
    {
            ...
            Action<string> errorHandler)
        {
            ...
            try
            {
                ...
                {
                    // Find all packages that are already installed
                    var installedPackages = new HashSet<PackageIdentity>(
                        (await nuGetProject.GetInstalledPackagesAsync(CancellationToken.None))
                        .Select(reference => reference.PackageIdentity));

                    var packageIdentity = new PackageIdentity(package.Id, package.Version);

                    // Does the project already have this package installed?
                    if (installedPackages.Any(installed => StringComparer.OrdinalIgnoreCase.Equals(package.Id, installed.Id)))
                    {
                        // If so, is it the right version?
                        if (!installedPackages.Contains(packageIdentity))
                        {
